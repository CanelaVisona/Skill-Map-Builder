...
export interface ErrorCelebrationState {
  name: string;
  // "detected" = toast chico al confirmar un error recién agregado.
  // "vencido" = la barra llegó a 50/50 -- celebración completa, igual que "Poder dominado".
  kind: "detected" | "vencido";
}

interface ErrorCelebrationProps {
  celebration: ErrorCelebrationState | null;
}

// Mismo estallido de partículas que PowerCelebration, pero con calaveras rojas en vez de
// estrellas doradas -- "vencido" es un evento negativo, no un logro.
...
export function ErrorCelebration({ celebration }: ErrorCelebrationProps) {
  ...
  if (celebration?.kind === "vencido") {
    return createPortal(
      <AnimatePresence>
        <motion.div
          key="error-celebration-vencido"
          ...
          <div className="relative flex items-center justify-center">
            {particles.map((p, i) => (
              ...
              >
                <Skull
                  size={p.size}
                  className="fill-red-500 text-red-500 drop-shadow-[0_0_6px_rgba(239,68,68,0.8)]"
                />
              </motion.div>
            ))}
            ...
            >
              ...
              >
                <Skull className="h-6 w-6 text-red-500" strokeWidth={2} />
              </motion.div>
