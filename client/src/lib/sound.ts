...
// Retro arcade "meter filling" sound -- used by the home-tasks list (NecesidadesCasa.tsx) so a
// task's bar visibly climbs in pitch as it fills, like a classic 8-bit score-tally or
// power-meter jingle (think Mario Bros. coin counter). Synthesized as a fast run of short
// square-wave blips spread evenly across `durationMs`, each pitched higher than the last --
// the climb is exponential rather than linear because pitch is perceived logarithmically, so
// an exponential sweep is what reads as a smooth, even "rise" to the ear.
export function playGrowingProgressBarSound(durationMs: number): { stop: () => void } {
  ...
  try {
    ...
    const startFreq = 300;
    const endFreq = 1300;
    const notesPerSecond = 14; // arcade-fast, distinct blips rather than a smooth siren
    const noteCount = Math.max(6, Math.round(duration * notesPerSecond));
    ...
    for (let i = 0; i < noteCount; i++) {
      ...
      gain.gain.linearRampToValueAtTime(0.12, noteStart + 0.008);
