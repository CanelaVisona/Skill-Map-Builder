...
export interface IStorage {
  ...
  updateUserTrackerOrder(userId: string, order: string[]): Promise<User | undefined>;
  ...
  updateQuestionItem(id: string, patch: Partial<Pick<InsertQuestionItem, "question" | "answer" | "action">>): Promise<QuestionItem | undefined>;
  deleteQuestionItem(id: string): Promise<void>;
...
export class DbStorage implements IStorage {
  ...
  async updateUserTrackerOrder(userId: string, order: string[]): Promise<User | undefined> {
    const result = await db.update(users).set({ trackerOrder: order }).where(eq(users.id, userId)).returning();
    return result[0];
  }
  ...
  /**
   * Recalculate node statuses for the entire area/project.
   * Rules based on unlockedLevel:
   * - Levels < unlockedLevel: all nodes are "locked" (completed levels)
   * - Level == unlockedLevel: levelPosition 1 is "mastered", first non-mastered is "available"
   * - Levels > unlockedLevel: all nodes are "locked" (future levels)
   */
  async recalculateNodeStatuses(
    ...
    // Get unlockedLevel from area or project
    if (areaId) {
      const area = await db.select().from(areas).where(eq(areas.id, areaId)).limit(1);
      if (!area.length) return;
      unlockedLevel = area[0].unlockedLevel || 1;
    } else if (projectId) {
      const project = await db.select().from(projects).where(eq(projects.id, projectId)).limit(1);
      if (!project.length) return;
      unlockedLevel = project[0].unlockedLevel || 1;
    } else {
      return; // Need either areaId or projectId
    }

    // Get all skills in the area/project/parent
    const allSkills = await db.select().from(skills).where(
      and(
        areaId ? eq(skills.areaId, areaId) : undefined,
        projectId ? eq(skills.projectId, projectId) : undefined,
        parentSkillId ? eq(skills.parentSkillId, parentSkillId) : undefined
    ...
    for (const skill of allSkills) {
      ...
      if (!skillsByLevel.has(lv)) {
        skillsByLevel.set(lv, []);
      }
      skillsByLevel.get(lv)!.push(skill);
    }

    // Process each level according to unlockedLevel
    for (const [lvl, lvlSkills] of skillsByLevel) {
      // Sort by levelPosition
      ...
      for (const skill of sortedByPosition) {
        ...
        if (lvl < unlockedLevel) {
          // All nodes in completed levels (below unlockedLevel) are mastered
          newStatus = "mastered";
        } else if (lvl === unlockedLevel) {
          // Current unlocked level: apply the progression rules
          ...
          // Levels above unlockedLevel: all locked
          newStatus = "locked";
        }
  ...
  async getJournalShadows(userId: string): Promise<JournalShadow[]> {
    return await db.select().from(journalShadows).where(eq(journalShadows.userId, userId)).orderBy(asc(journalShadows.createdAt));
  }
  ...
  async getSourceBugs(userId: string, type: "area" | "project", sourceId: string): Promise<Array<SourceBug & { registros: SourceBugRecord[] }>> {
    const bugs = type === "area"
      ? await db.select().from(sourceBugs).where(and(eq(sourceBugs.userId, userId), eq(sourceBugs.areaId, sourceId)))
      : await db.select().from(sourceBugs).where(and(eq(sourceBugs.userId, userId), eq(sourceBugs.projectId, sourceId)));
  ...
  async deleteSourceBug(id: string): Promise<void> {
  ...
  // los que se cargaron directo ahí (sin nodo, desde el Journal), Y los que se cargaron en
  // CUALQUIER OTRO nodo (o subskill, a cualquier profundidad) de esa misma área/proyecto -- así
  // un error agregado en un nodo queda disponible como opción para el resto de los nodos del área.
  async getNodeErrors(userId: string, skillId: string, scope?: { areaId?: string; projectId?: string }): Promise<NodeError[]> {
    ...
    if (scope?.areaId || scope?.projectId) {
      const scopedSkillIds = await this.getAllSkillIdsInScope({ areaId: scope.areaId, projectId: scope.projectId });
      if (scopedSkillIds.length > 0) {
        scopeConditions.push(inArray(nodeErrors.skillId, scopedSkillIds));
      }
    }
  ...
  async createFinancialGoal(goal: InsertFinancialGoal & { userId: string }): Promise<FinancialGoal> {
    const id = randomUUID();
    const result = await db.insert(financialGoals)
      .values({ id, ...goal } as any)
      .returning();
    return result[0];
  }
  ...
  async updateFinancialGoal(id: string, goal: Partial<InsertFinancialGoal>): Promise<FinancialGoal | undefined> {
    const result = await db.update(financialGoals)
      .set({ ...goal, updatedAt: new Date() } as any)
      .where(eq(financialGoals.id, id))
      .returning();
    return result[0];
  }
  ...
  // Preguntas (question problems + chained items)
  async getQuestionProblems(userId: string): Promise<QuestionProblemWithItems[]> {
    const problems = await db.select().from(questionProblems)
      .where(eq(questionProblems.userId, userId))
      .orderBy(asc(questionProblems.createdAt));
    if (problems.length === 0) return [];
    const items = await db.select().from(questionItems)
      .where(eq(questionItems.userId, userId))
      .orderBy(asc(questionItems.createdAt));
    const byProblem = new Map<string, QuestionItem[]>();
    for (const it of items) {
      const list = byProblem.get(it.problemId) ?? [];
      list.push(it);
      byProblem.set(it.problemId, list);
    }
    return problems.map((p) => ({ ...p, items: byProblem.get(p.id) ?? [] }));
  }

  async getQuestionProblem(id: string): Promise<QuestionProblem | undefined> {
    const result = await db.select().from(questionProblems).where(eq(questionProblems.id, id));
    return result[0];
  }

  async createQuestionProblem(problem: InsertQuestionProblem & { userId: string }): Promise<QuestionProblem> {
    const id = randomUUID();
    const result = await db.insert(questionProblems)
      .values({ id, ...problem } as any)
      .returning();
    return result[0];
  }
  ...
  async deleteQuestionProblem(id: string): Promise<void> {
    await db.delete(questionProblems).where(eq(questionProblems.id, id));
  }

  async getQuestionItem(id: string): Promise<QuestionItem | undefined> {
    const result = await db.select().from(questionItems).where(eq(questionItems.id, id));
    return result[0];
  }

  async createQuestionItem(item: InsertQuestionItem & { userId: string }): Promise<QuestionItem> {
    const id = randomUUID();
    const result = await db.insert(questionItems)
      .values({ id, ...item } as any)
      .returning();
    return result[0];
  }

  async updateQuestionItem(id: string, patch: Partial<Pick<InsertQuestionItem, "question" | "answer" | "action">>): Promise<QuestionItem | undefined> {
    const result = await db.update(questionItems)
      .set({ ...patch, updatedAt: new Date() } as any)
  ...
  async deleteQuestionItem(id: string): Promise<void> {
    await db.delete(questionItems).where(eq(questionItems.id, id));
  }
  ...
  // Evidence Board (Diario de pistas)
  async getEvidenceBoard(userId: string): Promise<EvidenceBoardRow | undefined> {
    const result = await db.select().from(evidenceBoards).where(eq(evidenceBoards.userId, userId)).limit(1);
    return result[0];
  }

  async upsertEvidenceBoard(userId: string, state: unknown): Promise<EvidenceBoardRow> {
    const existing = await db.select().from(evidenceBoards).where(eq(evidenceBoards.userId, userId)).limit(1);
    if (existing[0]) {
      const result = await db.update(evidenceBoards)
        .set({ state: state as any, updatedAt: new Date() })
        .where(eq(evidenceBoards.userId, userId))
        .returning();
      return result[0];
    }
    const result = await db.insert(evidenceBoards)
      .values({ userId, state: state as any, updatedAt: new Date() } as any)
      .returning();
    return result[0];
  }
  ...
  async getLifeGoals(userId: string, sourceType: "area" | "project", sourceId: string): Promise<LifeGoalsRow | undefined> {
    const id = `${userId}:${sourceType}:${sourceId}`;
    const result = await db.select().from(lifeGoals).where(eq(lifeGoals.id, id)).limit(1);
    return result[0];
  }
  ...
  // Meal Tracker (Mi Día)
  async getMealTrackerDay(userId: string, date: string): Promise<MealTrackerDay | undefined> {
    const id = `${userId}:${date}`;
    const result = await db.select().from(mealTrackerDays).where(eq(mealTrackerDays.id, id)).limit(1);
    return result[0];
  }
  ...
  async upsertMealTrackerDay(userId: string, date: string, data: Partial<Pick<InsertMealTrackerDay, "meals" | "regCelebrated" | "celebrated">>): Promise<MealTrackerDay> {
    ...
    if (existing[0]) {
      const result = await db.update(mealTrackerDays)
        .set({ ...data, updatedAt: new Date() } as any)
        .where(eq(mealTrackerDays.id, id))
        .returning();
      return result[0];
    }

    const now = new Date();
    const result = await db.insert(mealTrackerDays)
      .values({
        id,
        userId,
        date,
        meals: data.meals ?? {},
        regCelebrated: data.regCelebrated ?? {},
  ...
  async getMealTrackerCustomOptions(userId: string): Promise<MealTrackerCustomOption[]> {
    return await db.select().from(mealTrackerCustomOptions).where(eq(mealTrackerCustomOptions.userId, userId));
  }

  async createMealTrackerCustomOption(option: InsertMealTrackerCustomOption & { userId: string }): Promise<MealTrackerCustomOption> {
    const id = randomUUID();
    const result = await db.insert(mealTrackerCustomOptions).values({ id, ...option } as any).returning();
    return result[0];
  }

  async deleteMealTrackerCustomOption(userId: string, mealKind: string, categoryKey: string, name: string): Promise<void> {
    await db.delete(mealTrackerCustomOptions).where(
      and(
        eq(mealTrackerCustomOptions.userId, userId),
        eq(mealTrackerCustomOptions.mealKind, mealKind as "main" | "light"),
  ...
  async getMealTrackerMeta(userId: string): Promise<MealTrackerMeta | undefined> {
    const result = await db.select().from(mealTrackerMeta).where(eq(mealTrackerMeta.userId, userId)).limit(1);
    return result[0];
  }

  async upsertMealTrackerMeta(userId: string, data: { streak: number; lastCompleteDate: string }): Promise<MealTrackerMeta> {
    const existing = await db.select().from(mealTrackerMeta).where(eq(mealTrackerMeta.userId, userId)).limit(1);
    if (existing[0]) {
      const result = await db.update(mealTrackerMeta)
        .set({ ...data, updatedAt: new Date() })
        .where(eq(mealTrackerMeta.userId, userId))
        .returning();
      return result[0];
    }
    const result = await db.insert(mealTrackerMeta)
      .values({ userId, ...data, updatedAt: new Date() } as any)
      .returning();
    return result[0];
  }

  // Body Progress (Fuerza/Flexibilidad)
  async getBodyProgress(userId: string): Promise<BodyProgressRow[]> {
    return await db.select().from(bodyProgress).where(eq(bodyProgress.userId, userId));
  }

  async upsertBodyProgress(row: InsertBodyProgress): Promise<BodyProgressRow> {
    const existing = await db.select().from(bodyProgress).where(eq(bodyProgress.id, row.id)).limit(1);

    if (existing[0]) {
      const result = await db.update(bodyProgress)
        .set({ lvl: row.lvl, val: row.val, updatedAt: new Date() })
        .where(eq(bodyProgress.id, row.id))
        .returning();
      return result[0];
    }
