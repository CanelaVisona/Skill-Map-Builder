...
async function recalculateAvailableStatus(areaId: string, level: number) {
  ...
  // CRITICAL RULE: Position 1 (skeleton) MUST ALWAYS be mastered
  ...
  // Find first non-mastered (starting from position 2)
...
async function fixIntelectual() {
  console.log('=== FIXING INTELECTUAL AREA ===\n');

  const intelectualSkills = await db.select().from(skills).where(
    eq(skills.areaId, 'intelectual')
  ).orderBy(asc(skills.level), asc(skills.levelPosition));

  // Group by level
  const levelGroups = new Map<number, typeof intelectualSkills>();
  for (const skill of intelectualSkills) {
  ...
  const intelectualSkillsAfter = await db.select().from(skills).where(
    eq(skills.areaId, 'intelectual')
  ).orderBy(asc(skills.level), asc(skills.levelPosition));

  const levelGroupsAfter = new Map<number, typeof intelectualSkillsAfter>();
  for (const skill of intelectualSkillsAfter) {
